...
void* encode(void* loc,uint32_t totalSize,bool allocated){
    ...
    *val = 0;
    ...
    val = loc;
   /* //Lets print the whole number for once
    printf("TotalSize %d\n",copy);
...
uint32_t decodeSizeBack(void* loc){
    uint8_t* val = ++loc;
    uint32_t num = 0;
    if((0b00000011&*val)==0){
...
chunkDetails getDetailsBack(void* loc){//Will scan from right most part to get the details
    chunkDetails data;
    ...
    //The First block is the only one where second LSB is 0
    if((*iter&0b00000011)==0){//Means a case of only a single byte
        printf("Case Of 1 byte\n");
        data.memStart = iter-1;
        data.TotalSize =  decodeSizeBack(data.memStart);//It will do the heavylifting!
        data.metaSize = 1;
        data.availableSize = data.TotalSize-2;
        data.status = (*iter)&0b00000100;
    }
    ...
    if(!(*iter&0b10)){
        //Means 2 byte
        printf("Case Of 2 byte\n");
        data.memStart = iter-1;
        data.TotalSize =  decodeSizeBack(data.memStart);
        data.metaSize = 2;
        data.availableSize = data.TotalSize-4;
        data.status = (*iter)&0b00000100;
